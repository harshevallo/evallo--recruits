...
export function MockCompanyCard() {
  return (
    <div
      aria-hidden="true"
      className="rotate-2 transform rounded-xl border border-gray-200 bg-white p-6 shadow-lg transition-transform duration-300 hover:rotate-0"
    >
      <div className="mb-6 flex items-center gap-4">
        <Avatar initials="SL" size="lg" shape="rounded" tone="brand" className="shadow-inner" />
        <div>
          <p className="text-xl font-bold text-brand-dark">Seven Square Learning</p>
          <p className="text-sm text-gray-600">Test Prep &amp; Academic Tutoring</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {OPEN_ROLES.map((role) => (
          ...
          >
            <div>
              <p className="font-medium text-brand-dark">{role.title}</p>
              <p className="mt-1 text-xs text-gray-600">{role.meta}</p>
            </div>
